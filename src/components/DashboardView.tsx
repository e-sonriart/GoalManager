...
export const DashboardView: React.FC<DashboardViewProps> = ({ onNavigate }) => {
  ...
  const totalGoles = estadisticas.reduce((acc, s) => acc + (Number(s.goles) || 0), 0);
  const totalAsistencias = estadisticas.reduce((acc, s) => acc + (Number(s.asistencias) || 0), 0);
  const totalTarjetas = estadisticas.reduce((acc, s) => acc + (Number(s.tarjetas) || 0), 0);

  // Tasa de asistencia general
  const totalRegistrosAsistencia = asistencias.length;
  const asistenciasPositivas = asistencias.filter(a => a.estado === 'asiste').length;
  const porcentajeAsistencia = totalRegistrosAsistencia > 0
    ? Math.round((asistenciasPositivas / totalRegistrosAsistencia) * 100)
    : 100;

  // Pichichi (Máximo Goleador)
  const estadisticasOrdenadas = [...estadisticas].sort((a, b) => b.goles - a.goles);
  const topScorerStat = estadisticasOrdenadas[0];
  const topScorerPlayer = topScorerStat
    ? jugadores.find(j => j.id === topScorerStat.jugadorId)
    : null;
  ...
  const topGoleadoresData = estadisticasOrdenadas
    .slice(0, 5)
    .map(st => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {currentUser?.equipo && (currentUser.rol === 'entrenador' || currentUser.rol === 'jugador') && (
        <div className="bg-gradient-to-r from-orange-500/10 via-orange-500/5 to-transparent border border-orange-200/90 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xs animate-in fade-in duration-300">
          ...
          <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto">
            ...
            <button
              onClick={() => onNavigate(currentUser.rol === 'entrenador' ? 'asistencias' : 'partidos')}
            ...
            >
              {currentUser.rol === 'entrenador' ? 'Pasar Asistencia' : 'Ver Calendario'}
            </button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        ...
        <div className="bg-white p-3.5 sm:p-5 rounded-2xl border border-gray-150 shadow-xs flex flex-col sm:flex-row items-center sm:items-start gap-2.5 sm:gap-4">
          <div className="p-2 sm:p-3.5 rounded-xl sm:rounded-2xl bg-emerald-100 text-emerald-600 shrink-0">
            <Trophy className="w-5 h-5 sm:w-6 sm:h-6" />
          </div>
          <div className="text-center sm:text-left min-w-0">
            <p className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-gray-500 truncate">Goles Club</p>
            <h3 className="text-xl sm:text-2xl font-black text-gray-900 font-athletic">{totalGoles}</h3>
            <p className="hidden sm:block text-[11px] text-emerald-600 font-medium">+{totalAsistencias} asist.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Pichichi Spotlight */}
          {topScorerPlayer && (
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 rounded-3xl p-6 text-white shadow-lg relative overflow-hidden flex flex-col sm:flex-row items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-6 z-10 bg-black/20 backdrop-blur px-6 py-3 rounded-2xl border border-white/10">
                <div className="text-center">
                  <p className="text-[10px] uppercase font-bold text-orange-200">Goles</p>
                  <p className="text-3xl font-black font-athletic">{topScorerStat.goles}</p>
                </div>
                <div className="w-px h-8 bg-white/20" />
                <div className="text-center">
                  <p className="text-[10px] uppercase font-bold text-orange-200">Asistencias</p>
                  <p className="text-2xl font-bold font-athletic">{topScorerStat.asistencias}</p>
                </div>
                <div className="w-px h-8 bg-white/20" />
                <div className="text-center">
                  <p className="text-[10px] uppercase font-bold text-orange-200">Partidos</p>
                  <p className="text-2xl font-bold font-athletic">{topScorerStat.partidosJugados}</p>
                </div>
              </div>
            </div>
          )}
                  ...
                  <div key={idx} className="space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-semibold text-gray-800 flex items-center gap-2">
                        ...
                        {jug.nombre} <span className="text-gray-400 text-[11px]">(#{jug.dorsal})</span>
                      </span>
                      <div className="flex items-center gap-3">
                        <span className="font-bold text-orange-600 font-athletic text-sm">{jug.goles} Goles</span>
                        <span className="text-gray-400 text-[11px]">{jug.asistencias} Asist.</span>
                      </div>
                    </div>
